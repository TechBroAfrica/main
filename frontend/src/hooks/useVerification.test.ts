...
describe('useVerification – cancellation and race handling', () => {
  it('cancel sets CANCELLED and isVerifying false', async () => {
    ...
    vm.extractMetadata.mockReturnValue(new Promise((res) => { resolveExtract = res as (value: unknown) => void }))
    vm.fetchProofEventsByVideo.mockResolvedValue([])
    vm.getOnChainProof.mockResolvedValue(null)
  ...
  it('repeated verification does not let stale result overwrite newer', async () => {
    ...
    vm.extractMetadata.mockImplementationOnce(() => new Promise((res) => { firstExtractResolve = res as (value: unknown) => void }))
    vm.fetchProofEventsByVideo.mockImplementationOnce(() => new Promise((res) => { firstDbResolve = res as (value: unknown) => void }))
  ...
  it('retry re-runs last verification', async () => {
    ...
    vm.extractMetadata.mockResolvedValueOnce({ hasHarpocratesMetadata: true })
    vm.fetchProofEventsByVideo.mockResolvedValueOnce([{ id: 9, event_type: 'registered', file_name: null, video_hash: null, proof_id: null, tier: null, created_at: '' } as unknown as Record<string, unknown>])
    const { result } = renderHook(() => useVerification())
    await act(async () => { await result.current.verifyEvidence(makeVideoFile()) })
